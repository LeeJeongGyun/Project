...
private:
	...
	{
		stSession() :ioRefCount(0x01000000), bCancelFlag(false), maxTimeOutTick(50000)
		{
		}
...
private:
	...
	// NOTE
	// Interlocked함수를 사용하기 때문에 동일 캐시라인에 있으면 
	// 연관이 없음에도 lock이 끝나길 기다려야 되는 상황이 발생할 수 있다.
...
{
	// NOTE
	// ioRefCount의 상위 1비트는 현재 DisconnectSession이 실행됐는지 판단
